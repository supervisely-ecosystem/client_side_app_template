...
<div>
  ...
  methods: {
    ...
    shutdownApp() {
      console.log('--- SHUTDOWN TIMEOUT TRIGGERED', taskDataQueue.length);
      if (taskDataQueue.length) {
        console.log('--- SHUTDOWN TIMEOUT DELAYED', taskDataQueue.length);
        shutdownTimeout = setTimeout(this.shutdownApp, 1000);
        return;
      }

      console.log('--- SHUTDOWN TIMEOUT SEND REQUEST');
      this.post('/sly/session-info');
    },
    ...
    async merge(payload) {
      ...
      if (payload.action === 'shutdown') {
        ...
        console.log('--- SHUTDOWN TIMEOUT STARTED');
